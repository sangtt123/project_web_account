...
import { LayoutDashboard, Package, ShoppingCart, Terminal, Plus, RefreshCw, LogOut, User, Menu, X } from 'lucide-react';
        ...
        <div className="min-h-screen bg-gray-100 flex font-sans">
            ...
            <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-indigo-900 text-indigo-100 flex flex-col h-full shadow-xl transition-transform duration-300 ease-in-out md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
                ...
                <div className="p-6 border-b border-indigo-800 flex justify-between items-center">
                    ...
                    <button onClick={() => setIsSidebarOpen(false)} className="md:hidden text-indigo-300 hover:text-white">
                        <X className="w-6 h-6" />
                    </button>
                </div>
                ...
                <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
                    <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === 'overview' ? 'bg-indigo-700 text-white shadow-lg' : 'hover:bg-indigo-800'}`}
                    >
                        <LayoutDashboard className="w-5 h-5" /> Tổng quan
                    </button>
                    <button
                        onClick={() => { setActiveTab('orders'); setIsSidebarOpen(false); }}
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === 'orders' ? 'bg-indigo-700 text-white shadow-lg' : 'hover:bg-indigo-800'}`}
                    >
                        <ShoppingCart className="w-5 h-5" /> Đơn hàng
                    </button>
                    <button
                        onClick={() => { setActiveTab('stock'); setIsSidebarOpen(false); }}
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === 'stock' ? 'bg-indigo-700 text-white shadow-lg' : 'hover:bg-indigo-800'}`}
                    >
                        <Package className="w-5 h-5" /> Kho tài khoản
                    </button>
                    <button
                        onClick={() => { setActiveTab('logs'); setIsSidebarOpen(false); }}
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === 'logs' ? 'bg-indigo-700 text-white shadow-lg' : 'hover:bg-indigo-800'}`}
                    >
            ...
            <main className="flex-1 p-4 md:p-8 overflow-auto h-screen md:ml-64 bg-gray-100">
                {/* Header Bar (Mobile Toggle + Title) */}
                <div className="flex justify-between items-center mb-6 md:mb-8 sticky top-0 bg-gray-100 z-20 py-2">
                    <div className="flex items-center gap-3">
                        <button
                            onClick={toggleSidebar}
                            className="md:hidden p-2 bg-white rounded-lg shadow-sm border border-gray-200 text-gray-600 hover:bg-gray-50 active:bg-gray-100"
                        >
                ...
                {activeTab === 'overview' && stats && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 animate-in fade-in duration-500">
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                            ...
                            <p className="text-2xl md:text-3xl font-bold text-green-600 mt-2">{new Intl.NumberFormat('vi-VN').format(stats.totalRevenue)} đ</p>
                        </div>
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                            <p className="text-sm text-gray-500 uppercase tracking-wider font-semibold">Tổng đơn</p>
                            <p className="text-2xl md:text-3xl font-bold text-blue-600 mt-2">{stats.totalOrders}</p>
                        </div>
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                            <p className="text-sm text-gray-500 uppercase tracking-wider font-semibold">Đã bán</p>
                            <p className="text-2xl md:text-3xl font-bold text-indigo-600 mt-2">{stats.soldStock}</p>
                        </div>
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                            <p className="text-sm text-gray-500 uppercase tracking-wider font-semibold">Tồn kho</p>
                            <p className="text-2xl md:text-3xl font-bold text-orange-600 mt-2">{stats.availableStock}</p>
                        </div>
                    </div>
                )}
                        ...
                        {logs.map(log => (
                            <div key={log.id} className="mb-2 border-b border-gray-800 pb-2 last:border-0 hover:bg-gray-800/50 p-2 rounded transition-colors">
                                <div className="flex flex-col md:flex-row gap-1 md:gap-3 mb-1">
                                    <span className="text-gray-500 select-none text-xs">[{new Date(log.timestamp).toLocaleTimeString()}]</span>
                                ...
                                {log.details && (
                                    <pre className="text-xs text-gray-500 md:ml-[120px] whitespace-pre-wrap overflow-x-auto bg-gray-950 p-2 rounded">
