...
import { Zap, Search, SlidersHorizontal, X, ShieldCheck, Truck, Headphones, ChevronDown } from 'lucide-react';
...
export default function Home() {
    ...
    //----------------------------Catelogy---------------------------
    // 1. State lưu danh sách category từ API
    const [categories, setCategories] = useState<CategoryModel[]>([]);

    // 2. State lưu category đang active (Mặc định là 'All')
    const [activeCategory, setActiveCategoryId] = useState<String | 'All'>('All');

    // 3. Gọi API khi component được mount
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                // Lưu ý: Đường dẫn API là /api/catelogy (dựa trên folder structure)
                const res = await fetch('/api/categories');
                const data = await res.json();

                // Kiểm tra nếu data là mảng thì mới set state
    ...
    const [slides, setFetchSlides] = useState<Slides[]>([]);

    // Lấy toàn bộ data hero_slides
    useEffect(() => {
        const fetchSlides = async () => {
            try {
                const res = await fetch("/api/hero-slides"); // gọi API
                const data = await res.json();
                if (Array.isArray(data)) setFetchSlides(data);
            } catch (err) {
                console.error("Failed to fetch products:", err);
            }
        };
        ...
        <div className="min-h-screen bg-gray-50">
                                ...
                                return (
                                    <div key={slide.id} className="animate-in slide-in-from-left duration-700 fade-in fill-mode-forwards">
                                        ...
                                        <h1 className="text-2xl md:text-4xl lg:text-5xl font-extrabold mb-3 leading-tight">
                                            {slide.title} <br />
                                            <span className={`bg-clip-text bg-gradient-to-r ${slide.gradient}`}>
                                                {slide.highlight}
                                            </span>
                                        </h1>
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-3">
                                            <button
                                                ...
                                                className={`bg-white text-indigo-950 font-bold px-6 py-2.5 rounded-xl hover:bg-gray-100 transition-all shadow-lg shadow-white/10 text-center text-sm flex items-center justify-center gap-2`}
                                            >
            ...
            <section id="products" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 -mt-6">
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="bg-white p-3 rounded-xl shadow-lg shadow-gray-200/50 border border-gray-100 flex flex-col md:flex-row gap-4 items-center sticky top-20 z-40">
                        ...
                        <div className="relative flex-1 w-full">
                            ...
                            <input
                                ...
                                className="text-gray-500 w-full pl-9 pr-4 py-2 bg-gray-50 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm transition-all"
                            />
                            {searchQuery && (
                    ...
                    <div className="flex overflow-x-auto pb-2 gap-2 w-full scrollbar-hide">
                        {/* 4. Render nút "Tất cả" riêng biệt để dễ quản lý */}
                        <button
                            onClick={() => setActiveCategoryId('All')}
                            className={`whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-medium transition-all ${activeCategory === 'All'
                                ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200'
                                : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200'
                                }`}
                        >
                        ...
                        {/* 5. Map qua danh sách category từ API */}
                        {categories.map((cat) => (
                            <button
                                key={cat.category_name} // Dùng ID từ DB làm key
                                onClick={() => setActiveCategoryId(cat.category_name)}
                                className={`whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-medium transition-all ${activeCategory === cat.category_name
                                    ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200'
                                    : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200'
                                    }`}
                            >
                                {/* Hiển thị tên category từ DB */}
                                {cat.category_name}
                            </button>
                        ))}
                    </div>
                    ...
                    {filteredProducts.length > 0 ? (
                        <>
                            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                {filteredProducts.slice(0, visibleCount).map((product: Product, index) => {
                        ...
                        <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-200">
                            ...
                            <button
                                onClick={() => { setActiveCategoryId('All'); setSearchQuery(''); setMaxPrice(maxProductPrice) }}
