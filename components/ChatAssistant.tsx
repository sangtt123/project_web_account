...
import { generateShopResponse } from '../services/geminiService';
...
export const ChatAssistant: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { id: 'init', role: 'model', text: 'Hi! I can help you find the best subscription for your needs. Ask me anything!', timestamp: new Date() }
  ]);
  ...
  const handleSend = async () => {
    ...
    const responseText = await generateShopResponse(input);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
      <style>{`
        @keyframes chatPopIn { from { transform: scale(0.9) translateY(20px); opacity: 0; } to { transform: scale(1) translateY(0); opacity: 1; } }
